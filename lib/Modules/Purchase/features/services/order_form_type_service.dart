...
class OrderFormTypeService {
  static final OrderFormTypeService _instance =
    ...
    try {
      ...
      if (orderFormTypesData != null) {
        if (orderFormTypesData is List) {
          orderFormTypes = orderFormTypesData
              .map((item) {
                if (item is Map) {
                  // Ensure all values have the correct types
                  Map<String, dynamic> orderFormTypeMap =
                      Map<String, dynamic>.from(item);

                  // Ensure name is a string and not empty
                  if (orderFormTypeMap['name'] is! String ||
                      orderFormTypeMap['name'].toString().trim().isEmpty) {
                  ...
                  if (orderFormTypeMap['status'] is! String ||
                      orderFormTypeMap['status'].toString().trim().isEmpty) {
  ...
  Future<void> addOrderFormType(
    ...
    try {
      ...
      bool exists = orderFormTypes.any((type) => type['name'] == name);
      ...
      orderFormTypes.insert(0, {
        ...
        'isPlainMixed':
            isPlainMixed ?? false, // Default to false if not provided
  ...
  Future<void> updateOrderFormType(
    String oldName,
    String newName, {
    String? description,
    bool? isPlainMixed,
    ...
    try {
      ...
      int index = orderFormTypes.indexWhere((type) => type['name'] == oldName);
      ...
      bool exists = orderFormTypes.any(
        (type) => type['name'] == newName && type['name'] != oldName,
      );
      ...
      orderFormTypes[index] = {
        ...
        'description':
            description ??
            orderFormTypes[index]['description'], // Use existing description if not provided
        'isPlainMixed':
            isPlainMixed ??
            orderFormTypes[index]['isPlainMixed'], // Use existing isPlainMixed if not provided
        'status':
            status ??
            orderFormTypes[index]['status'], // Use existing status if not provided
      };
